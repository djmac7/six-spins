...
function toWebp(nativePath, q) {
  if (!HAVE_CWEBP) return
  const webp = nativePath.replace(/\.(jpg|png)$/i, '.webp')
  try {
    execFileSync('cwebp', ['-quiet', '-q', String(q), '-alpha_q', '100', nativePath, '-o', webp], { stdio: 'ignore' })
    rmSync(nativePath)
...
for (const fr of data.pool.franchises) {
  ...
  if (!code) continue
  tasks.push({ url: `https://a.espncdn.com/i/teamlogos/nba/500/${code}.png`, dest: join(pub, 'img', 'teams', `${fr.id}.png`), q: 90 })
}
...
async function run(task) {
  ...
  try {
    ...
    toWebp(task.dest, task.q) // -> .webp (~70% smaller); removes the native on success
    ok++
  } catch {
    miss++
  }
